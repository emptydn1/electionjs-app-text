      ...
      if (stop) {
        (async (textOrUrlManga, voice, speed, urlData, durationsTs) => {
          ...
          try {
            ...
            folderExist("pdf");
            folderExist("fixFile");
            appendNode(command, node, `--------start------`);
            scrollTerminal = setInterval(() => {
              const elem = document.querySelector(".command li");
            ...
            if (urlData !== "text") {
              const html = await rp.get(encodeURI(textOrUrlManga));
              $ = cheerio.load(html);
            ...
            appendNode(command, node, `--------HANDLE------`);
            ...
            if (dialogPath !== null) {
                    ...
                    return await axios({
                      ...
                      .catch((error) => {
                        appendNode(
                          command,
                          node,
                          `err recursionApi : ${count}`
                        );
                        console.log("err recursionApi", count);
                        return recursionApi(error, arrAPI, ++count);
                      });
                  } else {
                    appendNode(command, node, "err recursionApi not have AP");
              ...
              appendNode(
                command,
                node,
                `--SAVE VALUE OF ARRAYS FILE & CREATE FILE MP3-`
              );
              ...
              const checkError = async () => {
                appendNode(
                  command,
                  node,
                  `checkError${(arrErr.length, count_api)}`
                );
                ...
                if (arrErr.length) {
                  ...
                  appendNode(
                    command,
                    node,
                    `checkError END${(promises.length, count_api)}`
                  );
                  console.log(
                    "checkError END",
                    promises.length,
            ...
            document.querySelector(".command li").remove();
            stop = true;
            dialogPath = null;
            dialogPathPDF = null;
            contentHandle("done", textOrUrlManga);
            clearInterval(scrollTerminal);
          } catch (err) {
            if (document.querySelector(".command li"))
              document.querySelector(".command li").remove();
            stop = true;
            dialogPath = null;
            dialogPathPDF = null;
        ...
        dialog.showErrorBox("Error", "wait to handle files finish");
      }
    });
  },
  false
